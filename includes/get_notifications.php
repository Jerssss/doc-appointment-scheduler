...
try {
    ...
    foreach ($appointments as $app) {
        $acceptedAt = isset($app->accepted_at) ? $app->accepted_at->toDateTime()->format(DATE_ATOM) : null;


        // Fetch doctor info dynamically using doctor_id -> user_id
        $doctorName = "TBA";
        ...
        $results[] = [
            ...
            "notes"          => $app->notes ?? "",
            "time"           => isset($app->time) ? $app->time->toDateTime()->format(DATE_ATOM) : null,
            "accepted_at"    => $acceptedAt,
            "doctor_name"    => $doctorName,
            "type"           => ($app->status === 'in_progress' || $acceptedAt) ? "appointment_accepted" : "booking_created",
