document.addEventListener("DOMContentLoaded", function() {
    ...
    bookingForm.addEventListener("submit", async (e) => {
        ...
        const payload = {
            ...
            doctor_id: doctor.user_id?.$oid || doctor.user_id,
            mode: selectedMode,
            time: time,
            notes: notes,
        ...
        try {
           ...
           if (data.success) {
    alert("Booking successful! Please wait for the doctor to accept your appointment.");

    // Optional: store in notifications so patient can see it in notifications.html
    let notifications = JSON.parse(localStorage.getItem("notifications")) || [];
    notifications.push({
        type: "booking_created",
        message: `Your appointment with Dr. ${cleanName} is pending approval.`,
